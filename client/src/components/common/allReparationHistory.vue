...
const handleClick = async (reparationRow) => {
  const reparation =   (await axios.get(
    `http://localhost:3000/atelier/getReparationsById/${Number(reparationRow.idRep)}`
  )
).data
...
const mettreEnReparation = async () => {
   ...
   if(dDebRepRef.value!=null){
    ...
    ` http://localhost:3000/material/mettreEnReparation/${props.codeMat}`,req
  )
).data
console.log("success");
   }else{
  ...
  >
    ...
    <n-grid-item :span="5" class="div2">
        ...
        <NFormItemGi :span="16" label="detail">
          <NInput type="textarea" v-model:value="detailRef" resize="none"/>
        </NFormItemGi>
      </NGrid>
      <NSpace justify="end">
      </NSpace>
    ...
    <n-grid-item :span="5" class="div3"> 
        <NText style="font-size:15px;font-weight: bold;">la liste des reparations du materiel</NText>
        <STable @onRowClicked="handleClick" :data="reparations" :columns="cols" :border="true" style="margin-top: 15px;"/>
     </n-grid-item>
    </NGrid>
